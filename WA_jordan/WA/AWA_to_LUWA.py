...
def Reclass_LCC_to_LUWA(LCC_fh, Output_dir, ProtectedArea_tif, Reservoir_tif, LCC_LUWA_dict=None):
    ...
    if LCC_LUWA_dict is None:
        LCC_LUWA_dict = {
                      'wapor_classes': ['Code', 'Landuse', 'Wapor_description','Description','lu_new_classes'],
                                        ...
                                        10: ['MLU3', 'Modified', 'Cropland, rainfed', 'Rainfed pastures for grazing',35],
                                        11: ['ULU8', 'Utilized', 'Herbaceous cover', 'Herbaceous cover',15],
                                        12: ['ULU7', 'Utilized', 'Tree or shrub cover', 'Shrub land & mesquite',14],
                                        20: ['MWU3', 'Managed', 'Cropland, irrigated or post‐flooding', 'Irrigated crops - cereals',54],
                                        30: ['MLU12', 'Modified','Mosaic cropland (>50%) / natural vegetation (tree, shrub, herbaceous cover) (<50%)','Mixed species agro-forestry',44],
                                        40: ['ULU7', 'Utilized', 'Mosaic natural vegetation (tree, shrub, herbaceous cover) (>50%) / cropland (<50%)', 'Shrub land & mesquite',14],
                                        50: ['ULU3', 'Utilized', 'Tree cover, broadleaved, evergreen, closed to open (>15%)', 'Closed evergreen forest', 10],
                                        60: ['ULU1', 'Utilized', 'Tree cover, broadleaved, deciduous, closed to open (>15%)','Closed deciduous forest',8],
                                        ...
                                        80: ['ULU1', 'Utilized', 'Tree cover, needleleaved, deciduous, closed to open (>15%)', 'Closed deciduous forest',8],
                                        ...
                                        90: ['ULU4', 'Utilized' , 'Tree cover, mixed leaf type (broadleaved and needleleaved)','Open evergreen forest',11],
                                        100: ['ULU7', 'Utilized', 'Mosaic tree and shrub (>50%) / herbaceous cover (<50%)','Shrub land & mesquite',14],
    ...
    for key,values in Protected_dict.items():
        if isinstance(key, int):            
            mask = np.a.all([PLU, np.logical_or.reduce([LULC == x for x in values[1:][0]])], axis = 0)
