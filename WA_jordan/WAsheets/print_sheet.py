...
#    decimals = 1
    ...
    et_r_n = float(df_o.loc[(df_o.SUBCLASS == "ET RAIN") &
                             (df_o.VARIABLE == "Natural")].VALUE)
    et_r_u = float(df_o.loc[(df_o.SUBCLASS == "ET RAIN") &
                             (df_o.VARIABLE == "Urban")].VALUE)
    et_r_a = float(df_o.loc[(df_o.SUBCLASS == "ET RAIN") &
                             (df_o.VARIABLE == "Agri")].VALUE)

    et_i_n = float(df_o.loc[(df_o.SUBCLASS == "ET INCREMENTAL") &
                             (df_o.VARIABLE == "Natural")].VALUE)
    et_i_u = float(df_o.loc[(df_o.SUBCLASS == "ET INCREMENTAL") &
                             (df_o.VARIABLE == "Urban")].VALUE)
    et_i_a = float(df_o.loc[(df_o.SUBCLASS == "ET INCREMENTAL") &
                             (df_o.VARIABLE == "Agri")].VALUE)
...
#                                 (df_o.VARIABLE == "Natural")].VALUE)
    et_consumed = float(df_o.loc[(df_o.SUBCLASS == "ET INCREMENTAL") & #change this later, confusion regarding et
                                (df_o.VARIABLE == "Consumed Water")].VALUE)
#     sw_mrs_o = float(df_o.loc[(df_o.SUBCLASS == "SURFACE WATER") &
#                               (df_o.VARIABLE == "Main riverstem")].VALUE)
    total_sw_o = float(df_o.loc[(df_o.SUBCLASS == "SURFACE WATER") &
                          (df_o.VARIABLE == "Surface wateroutflow")].VALUE)
    ...
    treated_w_w = float(df_o.loc[(df_o.SUBCLASS == "OTHER") & #treated Waste Water being saved as outflow,other
                             (df_o.VARIABLE == "Treated Waste Water")].VALUE)
    ...
    # Light-green box
    green_et = et_r_n + et_r_u + et_r_a 
    
   
    # landscape et
    landsc_et = green_et + et_i_n + et_i_u 
   
    blue_et = et_i_n + et_i_u + et_i_a
   

   
    p2 = {
            'green_natural' : et_r_n,
            'green_urban' : et_r_u,
            'green_agriculture' : et_r_a,
            'blue_natural' : et_i_n,
            'blue_urban' : et_i_u,
            'blue_agriculture' : et_i_a,
            'green_et' : green_et,
            'blue_et' : blue_et
            }
...
#     p3 = {
#             'manamade_consumption' : et_manmade,
...
#    list(xml_txt_box)[0].text = '{1:.{0}f}'.format(decimals, other_o)
    ...
    consumed_water_ditl = et_consumed
    

#     outflow = net_inflow - green_et - blue_et
    return_flow = treated_w_w #return flow to river is treated waste water
    
    # Dark-green box
    
    consumed_manmade = et_i_a + consumed_water_ditl
    consumed_water = consumed_manmade + landsc_et
    mixed_sw = total_sw_o
    sw_o = mixed_sw - return_flow
    
    if sw_o < 0:
        sw_o = 0
        mixed_sw = return_flow
    
    p4 = {
#             'outflow' : outflow,
            'surface_water_outflow' : sw_o,
            'return_flow_to_river' : return_flow,
            'consumed_water_ditl' : consumed_water_ditl,
            'consumed_water' : consumed_water,
            'landscape_et' : landsc_et,
            'manmade_consumption' : consumed_manmade,
            'mixed_surface_outflow' : mixed_sw,
           # 'conserved_water' : mixed_sw,
            #'et_out' : consumed_water
            }
...
#                          p2['sp_r08_c01'] >= (1 - margin) * (p2['sp_r08_c02'] + p2['sp_r08_c03'])])])
    ...
    if data[0] is not None:
        ...
        #list(xml_txt_box)[0].text = 'Part 1: Manmade ({0})'.format(units[0])
        ...
        # request Marloes (remove Part 1 from title)
        if np.all([smart_unit, scale > 0]):
            list(xml_txt_box)[0].text = '({0} {1})'.format(10.**-scale, units[1])
        else:
            list(xml_txt_box)[0].text = '({0})'.format(units[1])
...
#    element7 = '<ns0:rect height="3.4300554" id="rect_low_mask" style="fill:#73b7d1;fill-opacity:1;stroke-width:0.26458332"  transform="translate(%.2f)" width="1.4658357" x="66.373032" y="267.59995" ns1:label="#rect8273" />'
    ...
    expected_nb_arrows = np.sum([len(dico_out[k]) for k in list(dico_out.keys())]) + np.sum([0 in dico_in[k] for k in list(dico_in.keys())])

    dico_in_b = {}
    for k in list(dico_in.keys()):
    ...
    if expected_nb_arrows == actual_nb_arrows:
        ...
        print('ERROR: unexpected number of arrows.\n      Basin structure too complex to generate Sheet 5 arrows automatically.\
               \n      Standard template returned as output.')
    return outpath


def print_sheet5(basin, sb_codes, dico_in, dico_out, period, units, data, output, template=False, smart_unit=False):
    ...
    wrong_path = os.path.join(*output.split('\\')[:-1])
    output_folder = wrong_path[:2]+'\\'+wrong_path[2:]
    svg_template_path = sheet_5_dynamic_arrows(dico_in, dico_out, svg_template_path,
...
#    current_variables = line_names
    for var1 in current_variables:
        ...
        for sb in sb_order:
            ...
            # fixing the -0.0 in DeltaS by increasing numb of digits
            if var1 == 'SW storage change':
                xml_txt_box[0].text = '%.2f' %(df_sb.VALUE)
            else:
...
def print_sheet6(basin, period, units, data, output, template=False, decimal = 1, smart_unit = False):
    ...
    list(xml_txt_box)[0].text = 'Sheet 6: Groundwater ({0})'.format(units)

    if np.all([smart_unit, scale > 0]):
        list(xml_txt_box)[0].text = 'Sheet 6: Groundwater ({0} {1})'.format(10**-scale, units)
    else:
        list(xml_txt_box)[0].text = 'Sheet 6: Groundwater ({0})'.format(units)
    ...
    for key in list(p1.keys()):
        xml_txt_box = tree1.findall(".//*[@id='{0}']".format(key))[0]
        if not pd.isnull(p1[key]):
            list(xml_txt_box)[0].text = '{1:.{0}f}'.format(decimal, p1[key])
        else:
            list(xml_txt_box)[0].text = '-'
