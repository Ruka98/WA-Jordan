...
def create_data_cube(metadata, nc_files,table_data):
    ...
    q_in_desal = table_data['q_in_desal']

    if q_in_sw != None or q_in_desal != None:
...
def collect_tables(folder,inflow = None ,outflow = None ,desal= None):
    table_data = {}
    if inflow == None:
        table_data['q_in_sw'] = None
    else:
        table_data['q_in_sw'] = os.path.join(folder,inflow)
   
    if outflow == None:
        table_data['q_outflow'] = None
    else:
        table_data['q_outflow'] = os.path.join(folder,outflow)
   
    if desal == None:
        table_data['q_in_desal'] = None
    else:
        table_data['q_in_desal'] = os.path.join(folder,desal)
